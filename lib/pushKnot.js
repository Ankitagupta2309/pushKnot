 ...
 */
function proxy(port, options, domain){
  ...
  socketServer.listen(8080, function() {
    log.info((new Date()) + ' Socket Server is listening on port 8080');
  });
  ...
  wsServer.on('request', function(request) {
    ...
    connection.on('message', function(message) {
      ...
      var filePath = path.join(__dirname, '..', 'content', 'response.json');
      fs.writeFileSync(filePath, message.utf8Data, {encoding : 'utf8'});
    });
  });
